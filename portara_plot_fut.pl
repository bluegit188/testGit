...
($#ARGV+2)==3 || die
"Usage: portara_plot_fut.pl SYM isAdj=0/1
...
my $isAdj =$ARGV[1];
    ...
    my $cmd="
cat /home/jgeng/RawData/portara/JunfCC/CCFixRTH/$sym.txt |gawk '{print \$1,\$5}' > /tmp/c.txt
...
{
$cmd="
cat /home/jgeng/RawData/portara/JunfCC/CCFixRTH/$sym.txt |gawk '{print \$1,\$5-\$12}' > /tmp/c.txt
datesXmgr.pl /tmp/c.txt 1 |mygetcols.pl 1 3 2 > /tmp/$sym.txt.plot
echo \"title \\\"$sym\\\"\" > /tmp/title.txt
xmgrByDate  -batch /tmp/title.txt /tmp/$sym.txt.plot&
";
