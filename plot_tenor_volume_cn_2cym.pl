...
my $DEBUG=0;
 ...
 ($#ARGV+2)==4  || die
"Usage: plot_tenor_volume_cn_2cym.pl SYM 2021K 2021M
       Plot volumes for specified tenor file for CN ETH files.
       For example, SYM=NI, ym=2021K 2021M etc\n";
...
# get the data
    my $cmd="
cat $file1 |mygetcols.pl 1 6 > /tmp/vol.txt
datesXmgr.pl /tmp/vol.txt  1|fgrep -v DATE|mygetcols.pl 1 3 > /tmp/forplot_vol.txt.1
cat $file2 |mygetcols.pl 1 6 > /tmp/vol.txt
datesXmgr.pl /tmp/vol.txt  1|fgrep -v DATE|mygetcols.pl 1 3 > /tmp/forplot_vol.txt.2
cat /home/jgeng/bin/batch_tenor_2lines.txt | sed s/SYM/$sym/  | sed s/YM1/$ym1/g| sed s/YM2/$ym2/g  > /tmp/batch_tenor_2cym.txt
xmgrByDate  -batch /tmp/batch_tenor_2cym.txt &
";
