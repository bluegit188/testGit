...
"Usage:reg_by_key_R.pl file.txt(header) colY colX colKey
       Compute simple OLS reg of y~x|key
...
for ( i in syms)
{
  ...
  fit1<-lm( $yName ~ $xName,data=regdataNow)
  summary(fit1)

  b0=summary(fit1)\$coefficients[,1][1]
  b1=summary(fit1)\$coefficients[,1][2]

  t0=summary(fit1)\$coefficients[,3][1]
  t1=summary(fit1)\$coefficients[,3][2]
